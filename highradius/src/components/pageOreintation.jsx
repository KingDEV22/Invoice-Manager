import React from "react";

const PageOreintation = (props) => {
  const { rowCount, itemCount, currentPage } = props;
  const pageCount = Math.ceil(itemCount / rowCount);
  const page = currentPage;
  const row = [10, 15, 20, 50, 100];

  const pageChangeArrow = (
    <p className="arrow">
      <svg
        onClick={() => {
          if (page > 0) props.onPageDecreamentChange(page, rowCount);
        }}
        xmlns="http://www.w3.org/2000/svg"
        width="20"
        height="20"
        fill={page === 0 ? null : "currentColor"}
        className="bi bi-arrow-left-circle-fill"
        viewBox="0 0 16 16"
      >
        <path d="M8 0a8 8 0 1 0 0 16A8 8 0 0 0 8 0zm3.5 7.5a.5.5 0 0 1 0 1H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5H11.5z" />
      </svg>
      <svg
        onClick={() => {
          if (page < pageCount) props.onPageIncreamentChange(page, rowCount);
        }}
        xmlns="http://www.w3.org/2000/svg"
        width="20"
        height="20"
        fill={page < pageCount ? "currentColor" : null}
        className="bi bi-arrow-right-circle-fill"
        viewBox="0 0 16 16"
      >
        <path d="M8 0a8 8 0 1 1 0 16A8 8 0 0 1 8 0zM4.5 7.5a.5.5 0 0 0 0 1h5.793l-2.147 2.146a.5.5 0 0 0 .708.708l3-3a.5.5 0 0 0 0-.708l-3-3a.5.5 0 1 0-.708.708L10.293 7.5H4.5z" />
      </svg>
    </p>
  );
  const dropMenu = row.map((number) => {
    return <option key={number}>{number}</option>;
  });

  return (
    <div className="page">
      <p>Rows per Page</p>
      <select className="dropdown" onChange={props.handleValue}>
        {dropMenu}
      </select>
      {pageChangeArrow}
      <p>
        {currentPage} - {pageCount} of {itemCount}
      </p>
    </div>
  );
};

export default PageOreintation;
